...
## [2024-12-19] - Анализ уведомлений в Telegram о несоответствии стока

### Добавлено
- Анализ всех мест в коде, где отправляются уведомления в Telegram о проблемах со стоком

### Найдено
- **Основные места отправки уведомлений о стоке:**
  - `app/services/stock_service.py` - уведомления при отсутствии товара в базе и нулевых остатках
  - `app/services/allegro/sync_tasks.py` - уведомления об ошибках обновления офферов Allegro
  - `app/celery_app.py` - уведомления об ошибках обработки заказов и событий
  - `app/services/tg_client.py` - основной класс для отправки сообщений в Telegram

### Детализация
- **Уведомления о стоке в stock_service.py:**
  - Товар не найден в базе данных
  - Нулевые остатки на складе A
  - Ошибки при списании товара

- **Уведомления в sync_tasks.py:**
  - Ошибки обновления офферов Allegro после исчерпания попыток
  - Информация об аккаунте, SKU, Offer ID, текущем и целевом стоке

- **Уведомления в celery_app.py:**
  - Ошибки обработки событий заказов
