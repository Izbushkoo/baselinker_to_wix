...
## [2025-01-05] - Добавление API роута для запуска синхронизации Wix
### Добавлено
- Новый API эндпоинт `POST /api/v1/allegro_sync/wix-sync` для единоразового запуска синхронизации стоков Wix
- Интеграция функции `launch_wix_sync` в API роутер
- Возврат ID задачи для отслеживания статуса выполнения

### Технические детали
- Роут добавлен в существующий файл `allegro_sync.py`
- Возвращает JSON с информацией о запущенной задаче
- Обработка ошибок с логированием

## [2025-01-05] - Исправление загрузки переменных окружения в Celery worker
### Добавлено
- Загрузка переменных окружения из `.env.docker` в начале файла `celery_app.py`
- Проверка загрузки переменных Wix в функциях `launch_wix_sync` и `sync_wix_inventory`
- Логирование статуса загрузки переменных окружения

### Исправлено
- Проблема с отсутствующими переменными WIX_API_KEY, WIX_SITE_ID в Celery worker
- Ошибка 500 от Wix API из-за пустых заголовков авторизации
- Загрузка переменных окружения в Docker контейнере

### Технические детали
- Добавлен импорт `dotenv` и логика загрузки `.env.docker` в Docker окружении
- Проверка переменных окружения перед созданием WixApiService
- Возврат ошибки с описанием проблемы при отсутствии критических переменных

## [2025-01-05] - Диагностика ошибки 404 от Wix API
### Добавлено
- Метод `test_connection()` для проверки подключения к Wix API
- API эндпоинт `GET /api/v1/allegro_sync/wix-test-connection` для тестирования подключения
- Подробное логирование инициализации WixApiService
- Тестирование альтернативных endpoints при ошибках

### Изменено
- Добавлена проверка подключения к Wix API перед синхронизацией
- Улучшена диагностика проблем с креденшиалами и site_id
- Добавлена валидация критических параметров в конструкторе WixApiService

### Технические детали
- Проверка валидности WIX_API_KEY и WIX_SITE_ID при инициализации
- Тестирование подключения с простым запросом перед основной синхронизацией
- Попытка использования альтернативного endpoint при ошибках 404
