...
### Обзор
Централизованная система управления складами с возможностью переименования, мониторинга и анализа данных по всем связанным таблицам.

### Архитектура
1. **Скрипты управления**: 
   - `scripts/warehouse_manager.py` - универсальный менеджер складов
   - `scripts/rename_warehouse.py` - специализированный скрипт для переименования
2. **Makefile команды**: Интеграция с системой сборки проекта
3. **Автоматическое обновление**: Обновление кода и базы данных в одной операции

### Функциональность
- **Переименование складов**: Безопасное переименование с обновлением всех связанных таблиц
- **Предварительный просмотр**: Режим dry-run для анализа изменений без применения
- **Статистика складов**: Детальная информация о количестве записей в каждой таблице
- **Список складов**: Автоматическое обнаружение всех складов в системе
- **Интеграция с кодом**: Автоматическое обновление перечисления Warehouses

### Поддерживаемые таблицы
- `stock` - остатки товаров на складах
- `sale` - логи продаж (списаний)
- `transfer` - логи перемещений между складами
- `operation` - операции с товарами
- `pending_stock_operation` - ожидающие операции синхронизации
...
### Текущий статус
- [x] Реализован универсальный менеджер складов
- [x] Добавлены команды в Makefile
- [x] Созданы скрипты для переименования
- [x] Интеграция с системой сборки проекта
...
1. **Сервис**: `PricesService` - основной класс для работы с БД цен
